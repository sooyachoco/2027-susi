...
function groupForMajor(major: string): MajorGroup | undefined {
  return MAJOR_GROUPS.find((group) => group.options.includes(major));
}

export function StudentProfileTemplateForm({ profile, onChange }: Props) {
  const selectedGroup = groupForMajor(profile.desiredMajor)?.label ?? "";
  const selectedOptions = MAJOR_GROUPS.find((group) => group.label === selectedGroup)?.options ?? [];
  ...
  return <div className="panel form">
    ...
    <div className="field">
      <label htmlFor="majorGroup">희망 전공 계열</label>
      <select id="majorGroup" value={selectedGroup} onChange={(e) => onChange({ desiredMajor: e.target.value ? "" : "" })}>
        <option value="">계열을 선택하세요</option>
        {MAJOR_GROUPS.map((group) => <option key={group.label} value={group.label}>{group.label}</option>)}
      </select>
    </div>

    <div className="field">
      <label htmlFor="major">세부 전공</label>
      <select id="major" value={profile.desiredMajor} disabled={!selectedGroup} onChange={(e) => onChange({ desiredMajor: e.target.value })}>
        <option value="">세부 전공을 선택하세요</option>
        {selectedOptions.map((major) => <option key={major} value={major}>{major}</option>)}
      </select>
    </div>
