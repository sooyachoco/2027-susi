...
function rotateBySeed<T>(items: T[], seed: number): T[] {
  if (items.length <= 1 || seed === 0) return items;
  const step = (seed * 3 + 1) % items.length;
  return items.slice(step).concat(items.slice(0, step));
}

function scoreVariation(seed: number, id: string): number {
  if (seed === 0) return 0;
  let hash = seed * 31;
...
function strategicAdjustment(student: StudentProfile, admission: Admission) {
  const gradeAverage = student.gradeAverage ?? 3, recordLink = student.studentRecordLink ?? 3, csatChance = student.csatMinimumChance ?? 3;
  let adjustment = 0;
...
function buildReason(admission: Admission, score: number) {
  ...
  if (admission.interview) parts.push("면접 변수 있음"); if (admission.csatMinimum?.enabled) parts.push("수능최저 반영");
  if (admission.source?.type === "adiga" || admission.source?.type === "university") parts.push("2027 전형 데이터"); if (admission.isMock) parts.push("프로토타입 데이터");
