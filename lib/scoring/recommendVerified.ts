...
export function recommendSix(student: StudentProfile, admissions: Admission[], offset = 0): Recommendation[] {
  ...
  const majorMatched = desired
    ? admissions.filter((admission) => majorNameMatches(desired, admission))
    : admissions;
  ...
  return diversifyUniversities(scored, 6).map((item) => ({
    tier: tierForScore(item.score), admissionId: item.admission.id, score: item.score,
...
function majorNameMatches(query: string, admission: Admission): boolean {
  ...
  return names.some((name) => {
    const tags = majorTags(normalizeText(name));
    return queryGroups.some((group) => tags.includes(group));
  });
}

function findDepartment(id: string): Department | undefined {
  return [...verified2027Departments, ...expanded2027Departments, ...remainingMetro2027Departments]
...
/** 복합전공까지 놓치지 않도록 학과명에서 의미 태그를 추출한다. */
function majorTags(value: string): string[] {
  const v = normalizeText(value);
  const tags: string[] = [];
  const add = (tag: string, patterns: RegExp) => { if (patterns.test(v)) tags.push(tag); };
  ...
  add("전기·전자", /전기|전자|반도체|통신|임베디드|전력|제어/);
  add("기계·로봇", /기계|자동차|로봇|메카트로닉스|모빌리티|항공우주/);
  add("화학·신소재", /화학|화공|신소재|재료|고분자|에너지|소재/);
  add("생명·바이오", /생명|바이오|식품|생물|유전|환경생명/);
  add("건축·도시·환경", /건축|토목|도시|환경|건설|조경|인프라/);
  add("자연과학", /수학|통계|물리|천문|지구과학|과학/);
...
function diversifyUniversities(items: Array<{ admission: Admission; score: number }>, limit: number) {
  ...
  for (const item of items) {
    ...
    if (!used.has(item.admission.universityId)) { selected.push(item); used.add(item.admission.universityId); }
  }
...
function strategicAdjustment(student: StudentProfile, admission: Admission) {
  ...
  const recordLink = student.studentRecordLink ?? 3;
  const csatChance = student.csatMinimumChance ?? 3;
  let adjustment = 0;
...
export function nextShuffleOffset(offset: number): number { return offset === 0 ? 3 : offset === 3 ? -2 : 0; }
