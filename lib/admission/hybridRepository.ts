...
export class HybridAdmissionRepository implements AdmissionRepository {
  private getVerifiedAdmissions(): Admission[] { return dedupeByKey([...remainingMetro2027Admissions,...sungkyul2027Admissions,...uos2027Admissions,...skku2027Admissions,...hanyang2027Admissions,...soongsil2027Admissions,...kookmin2027Admissions,...dankook2027Admissions].filter(a=>a.academicYear===2027&&!a.isMock)); }
  async getUniversities(region?: AdmissionRegion): Promise<University[]> { const allowed=new Set(this.getVerifiedAdmissions().map(a=>a.universityId)); const all=dedupeByKey([...universities,...expanded2027Universities,...remainingMetro2027Universities,...uos2027Universities,...skku2027Universities,...hanyang2027Universities,...soongsil2027Universities,...kookmin2027Universities,...dankook2027Universities]); const scoped=all.filter(u=>isTargetRegion(u.region)&&allowed.has(u.id)); const map=new Map<string,RootUniversity>(); for(const u of scoped){const key=`${normalize(u.name)}|${u.region}`;if(!map.has(key))map.set(key,{id:u.id,name:u.name,region:u.region});} const result=[...map.values()].filter(u=>!region||u.region===region); return result.map(u=>({id:u.id,name:u.name,region:u.region as AdmissionRegion})); }
  async getDepartments(universityId?: string): Promise<Department[]> { const allowed=new Set(this.getVerifiedAdmissions().map(a=>a.departmentId)); const all=dedupeByKey([...departments,...expanded2027Departments,...remainingMetro2027Departments,...sungkyul2027Departments,...uos2027Departments,...skku2027Departments,...hanyang2027Departments,...soongsil2027Departments,...kookmin2027Departments,...dankook2027Departments]); const verifiedOnly=all.filter(d=>allowed.has(d.id)); return universityId?verifiedOnly.filter(d=>d.universityId===universityId):verifiedOnly; }
