...
export class HybridAdmissionRepository implements AdmissionRepository {
  private getVerifiedAdmissions(): Admission[] {
    return dedupeByKey([
      ...remainingMetro2027Admissions.filter((a) => !["hanshin","seoultech","sejong","hongik","myeongji","sungshin","snue","dongduk","sahmyook","skuniv","gangseo"].includes(a.universityId)),
      ...seoulMid2027Admissions, ...seoulLower2027Admissions, ...gangseo2027Admissions,
  ...
  async getUniversities(region?: AdmissionRegion): Promise<University[]> {
    ...
    const all = dedupeByKey([...universities, ...expanded2027Universities, ...remainingMetro2027Universities, ...seoulMid2027Universities, ...seoulLower2027Universities, ...gangseo2027Universities,
  ...
  async getDepartments(universityId?: string): Promise<Department[]> {
    ...
    const all = dedupeByKey([...departments, ...expanded2027Departments, ...remainingMetro2027Departments.filter((d) => !["seoultech","sejong","hongik","myeongji","sungshin","snue","dongduk","sahmyook","skuniv","gangseo"].includes(d.universityId)),
      ...seoulMid2027Departments, ...seoulLower2027Departments, ...gangseo2027DepartmentsWithAggregate, ...sungkyul2027Departments, ...uos2027Departments, ...skku2027Departments, ...hanyang2027Departments, ...soongsil2027Departments,
