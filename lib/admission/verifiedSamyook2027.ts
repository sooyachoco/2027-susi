...
const source = { type: "university" as const, academicYear: 2027, url: "https://ipsi.syu.ac.kr/2016_syu/pages/index.asp?mj=01&p=8", document: "2027학년도 수시 모집요강", verifiedAt: "2026-09-06", confidence: 0.99 };
...
const noRegionalCsat = new Set(["physical-education", "art-design"]);
...
export const verifiedSamyook2027Admissions: Admission[] = verifiedSamyook2027Departments.flatMap((d) => {
  ...
  const out: Admission[] = [];
  const schoolMin = key === "pharmacy"
    ? "약학과: 국어·영어·수학(미적분 또는 기하)·과학탐구(1과목) 중 3개 영역 합 5등급 이내"
    : key === "nursing" || key === "physical-therapy"
      ? "간호학과·물리치료학과: 국어·영어·수학·탐구(1과목) 중 2개 영역 합 6등급 이내"
      : noRegionalCsat.has(key)
        ? "수능최저 없음"
        : "일반학과: 국어·영어·수학·탐구(1과목) 중 2개 영역 합 7등급 이내";
  out.push({ id: `${d.id}-school-recommend-2027`, universityId: d.universityId, departmentId: d.id, academicYear: 2027, name: "학교장추천", type: "교과", studentRecordWeight: 100, source, isMock: false, csatMinimum: { enabled: !noRegionalCsat.has(key), description: schoolMin } });
  ...
  if (!essayExcluded.has(key)) out.push({ id: `${d.id}-essay-2027`, universityId: d.universityId, departmentId: d.id, academicYear: 2027, name: "논술우수자", type: "논술", source, isMock: false, csatMinimum: { enabled: true, description: key === "pharmacy" ? "약학과: 국어·영어·수학(미적분 또는 기하)·과학탐구(1과목) 중 3개 영역 합 5등급 이내" : "일반학과: 국어·영어·수학·탐구(1과목) 중 1개 영역 3등급 이내" } });
  if (practical.has(key)) out.push({ id: `${d.id}-practical-2027`, universityId: d.universityId, departmentId: d.id, academicYear: 2027, name: "예체능인재", type: "기타", source, isMock: false });
  return out;
});
...
export const verifiedSamyook2027AggregateAdmissions: Admission[] = [
  { id: "samyook-2027-overall", universityId: "samyook", departmentId: verifiedSamyook2027AggregateDepartment.id, academicYear: 2027, name: "수시 전체", type: "기타", recruitmentCount: 1029, source, isMock: false },
  ...
  { id: "samyook-2027-overall-seum", universityId: "samyook", departmentId: verifiedSamyook2027AggregateDepartment.id, academicYear: 2027, name: "세움인재(전체)", type: "학종", recruitmentCount: 231, documentWeight: 60, interview: true, csatMinimum: { enabled: true, description: "약학과만 수능최저: 국어·영어·수학(미적분 또는 기하)·과학탐구(1과목) 중 3개 영역 합 5등급 이내" }, source, isMock: false },
  { id: "samyook-2027-overall-essay", universityId: "samyook", departmentId: verifiedSamyook2027AggregateDepartment.id, academicYear: 2027, name: "논술우수자(전체)", type: "논술", recruitmentCount: 277, csatMinimum: { enabled: true, description: "일반학과: 국어·영어·수학·탐구(1과목) 중 1개 영역 3등급 이내; 약학과는 3개 영역 합 5등급 이내" }, source, isMock: false },
];
