...
const verifiedUniversities = [...seoulRealBatch2Universities, ...seoulRealBatch3Universities, ...seoulRealBatch4Universities, ...seoulRealBatch5Universities, ...seoulRealBatch6Universities];
const verifiedDepartments = [...seoulRealBatch2Departments, ...seoulRealBatch3Departments, ...seoulRealBatch4Departments, ...seoulRealBatch5Departments, ...seoulRealBatch6Departments];
const verifiedAdmissions = [...seoulRealBatch2Admissions, ...seoulRealBatch3Admissions, ...seoulRealBatch4Admissions, ...seoulRealBatch5Admissions, ...seoulRealBatch6Admissions];
...
export class MockAdmissionRepository implements AdmissionRepository {
  async getUniversities(region?: AdmissionRegion) {
    ...
    return uniqueById([...universities, ...verifiedUniversities]).filter((u) => scope.includes(u.region));
  }
  async getDepartments(universityId?: string) {
    const all = uniqueById([...departments, ...verifiedDepartments]);
  ...
  async getAdmissions(query: AdmissionQuery = {}) {
    ...
    const allAdmissions = uniqueById([...admissions, ...verifiedAdmissions]);
    const allUniversities = uniqueById([...universities, ...verifiedUniversities]);
    const allowedUniversityIds = new Set(allUniversities.filter((u) => scope.includes(u.region)).map((u) => u.id));
    return allAdmissions.filter((a) => (!query.academicYear || a.academicYear === query.academicYear) && allowedUniversityIds.has(a.universityId) && (!query.universityId || a.universityId === query.universityId) && (!query.departmentId || a.departmentId === query.departmentId) && (!query.type || a.type === query.type));
  }
}
