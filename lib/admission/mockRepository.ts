...
import {
  seoulRealBatch2Admissions,
  seoulRealBatch2Departments,
  seoulRealBatch2Universities,
} from "./verified/seoul_real_batch2_2027";
import {
  seoulRealBatch3Admissions,
  seoulRealBatch3Departments,
  seoulRealBatch3Universities,
} from "./verified/seoul_real_batch3_2027";
import {
  seoulRealBatch4Admissions,
  seoulRealBatch4Departments,
  seoulRealBatch4Universities,
} from "./verified/seoul_real_batch4_2027";
import {
  seoulRealBatch5Admissions,
  seoulRealBatch5Departments,
  seoulRealBatch5Universities,
} from "./verified/seoul_real_batch5_2027";
...
export class MockAdmissionRepository implements AdmissionRepository {
  ...
  async getAdmissions(query: AdmissionQuery = {}) {
    ...
    const allowedUniversityIds = new Set(
      allUniversities.filter((u) => scope.includes(u.region)).map((u) => u.id)
    );
