...
const names = [
  "LIONS자율전공학부(전계열)", "LIONS자율전공학부(인문사회계열)", "LIONS자율전공학부(자연계열)", "글로벌문화통상학부",
...
const interviewDepartment = verifiedHanyangErica2027Departments.find(
  (department) => department.name === "LIONS자율전공학부(전계열)"
);

export const verifiedHanyangErica2027Admissions: Admission[] = [
  ...verifiedHanyangErica2027Departments
    .filter((department) => department.name !== "LIONS자율전공학부(전계열)")
    .flatMap((department) => [
      {
  ...
  ...(interviewDepartment ? [{
    id: `${interviewDepartment.id}-holistic-interview`, universityId: "hanyang-erica-2027", departmentId: interviewDepartment.id,
