...
export function isRegularProfileComplete(profile: RegularStudentProfile): boolean {
  return Boolean(profile.desiredMajor.trim() && profile.koreanStandard !== null && profile.koreanPercentile !== null && profile.mathStandard !== null && profile.mathPercentile !== null && profile.inquiry1Percentile !== null && profile.inquiry2Percentile !== null && profile.englishGrade !== null && profile.koreanHistoryGrade !== null && profile.mathChoice !== "미선택" && profile.inquiryType !== "미선택");
}
...
export function recommendRegular(profile: RegularStudentProfile, admissions: RegularAdmission[]): RegularRecommendation[] {
  ...
  for (const item of ranked) {
    ...
    if (!groups.has(item.admission.group)) { selected.push(item); groups.add(item.admission.group); }
  }
  for (const item of ranked) {
    if (selected.length >= 3) break;
  ...
  return selected.map(({ admission, score }) => ({
    ...
    reason: `${admission.group}군 · ${admission.department} · 입력 백분위 기반 모의지원 지수 ${score}`,
  }));
}
